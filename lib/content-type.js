...
/**
 * Insert or update comment into the database.
 *
 * @returns {Promise<int|error>}
 */
const setComment = async function( type, comment ) {
    ...
    if ( ! isEdit && ! comment.authorId ) {
        let {author, authorEmail} = comment;

        if ( (! author && ! authorEmail ) || ! isUserLoggedIn() ) {
            return reject( __t('No comment author specified.') );
        }
