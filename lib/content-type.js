...
/**
 * Returns an object use to create or update content tables.
 *
 * @access private
...
/**
 * Update content tables structure.
 *
 * @param {object} contentType
...
const updateContentTable = async function( contentType, oldContentType ) {
    let tableFields = getTableFields(contentType),
        tables = Object.keys(tableFields),
        newSlug = contentType.slug,
        oldSlug = oldContentType.slug;

    if ( newSlug !== oldSlug ) {
        let oldTableFields = getTableFields(oldContentType),
            oldTables = Object.keys(oldTableFields);

        for ( let i = 0; i < oldTables.length; i++ ) {
            let oldTable = oldTables[i];

            // Rename table
            //await dbManager.renameTable( oldTable )
        }
    }
...
const dropContentTable = function( contentType ) {
    ...
    getContents({type: slug}).then(handleResponse);
};
...
const setContentType = async function( args, oldSlug ) {
    ...
    let oldContentType;
    if ( oldSlug ) {
        oldContentType = await getContentType(oldSlug);
    } else {
        oldContentType = await getContentType(slug);
    }
    ...
    let handleResponse = async res => {
        ...
        if ( isEdit ) {
            // Clear cache
            contentCache.clear(oldContentType.slug);

            await updateContentTable( args, oldContentType );
             ...
             */
            await appEvent('updatedContentType').trigger( args, oldContentType );
...
/**
 * Add a new content type into the database.
 *
 * @param {object} args {
 *     @param {string} name
 *     @param {string} slug
